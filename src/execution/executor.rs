...
impl Executor {
    ...
    ///
    /// # Arguments
    /// * `dna` - The DNA to execute
    /// * `input` - Input values for the program
    /// * `primitive_registry` - Registry of available primitives
    /// * `template_registry` - Template library from DNA's lineage
    pub fn execute(
        &self,
        dna: &DNA,
        input: Vec<i64>,
...
mod tests {
    ...
    fn test_simple_execution() {
        ...
        let result = executor.execute(&dna, vec![42], &primitive_registry, &template_registry);
    ...
    fn test_arithmetic_execution() {
        ...
        let result = executor.execute(&dna, vec![], &primitive_registry, &template_registry);
    ...
    fn test_template_execution() {
        ...
        // Create a template registry for this lineage
        let mut template_registry = TemplateRegistry::new();
        let template_hash = template_registry.register(template_genes, 1.0, 0);
        ...
        let result = executor.execute(&dna, vec![21], &primitive_registry, &template_registry);
